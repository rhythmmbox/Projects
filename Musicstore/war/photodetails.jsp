...
<%@ page import="static com.musicstore.database.Ofyservice.*, com.musicstore.entity.User,com.musicstore.entity.Photo,java.io.*,javax.servlet.*,java.util.*" %>
...
<button class="w3-button w3-ripple w3-yellow"  onclick="window.location.href='/logout'" style="margin-right:5px">Logout</button>
    ...
    <div class="container">
 <h1 style="color:IndianRed;"><b>Photo</b></h1>
  <table class="table table-bordered">
    <thead>
      <tr>
      <th style="color:IndianRed;" >Photo Id(ID)</th>
        <th style="color:IndianRed;">LOCATION</th>
                <th style="color:IndianRed;">URL</th>
    ...
    <tbody><%
          ...
          List<Photo> p1=ofy().load().type(Photo.class).list();
				
				Iterator<Photo> Fetch1= p1.iterator();
				while(Fetch1.hasNext())
				{
					Photo pd1=Fetch1.next();
					
				String substring=pd1.getP_id().substring(0,pd1.getP_id().indexOf('-')+1);
      ...
      <tr>
            ...
            <%if(substring.length()>5) {%>              <td><%=substring%></td>
            <%} else{ %>
      		<td><%=pd1.getP_id()%></td><%} %>
        <td><%=pd1.getLoc()%></td>
        <td><%=pd1.getUrl()%></td>
